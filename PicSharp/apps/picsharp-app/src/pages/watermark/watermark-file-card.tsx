...
interface WatermarkFileCardProps {
  path: string;
}

function WatermarkFileCard({ path }: WatermarkFileCardProps) {
  const update = useUpdate();
  const { fileMap, eventEmitter } = useWatermarkStore();
  ...
  useEffect(() => {
    ...
    return () => {
      eventEmitter.off('update_file_item', handler);
    };
  }, [eventEmitter, update]);
  ...
  const src =
    file.status === WatermarkStatus.Completed && file.outputPath
  ...
  return (
    <div
      className={cn(
        ...
        file.status === WatermarkStatus.Failed && 'border-destructive/50',
      )}
    >
      <div className='relative aspect-[4/3] w-full overflow-hidden bg-neutral-100 dark:bg-neutral-700'>
        <ImageViewer
          src={src}
          size={file.bytesSize}
          path={file.status === WatermarkStatus.Completed && file.outputPath ? file.outputPath : file.path}
          ext={file.ext}
        ...
        <div className='absolute right-1 top-1 flex items-center gap-0.5 rounded bg-black/50 px-1.5 py-0.5 text-[10px] text-white'>
          {file.status === WatermarkStatus.Processing && (
            <Loader2 className='h-3 w-3 animate-spin' />
          )}
          {file.status === WatermarkStatus.Completed && (
            <Check className='h-3 w-3 text-green-400' />
          )}
          {file.status === WatermarkStatus.Failed && (
      ...
      <div className='flex flex-col gap-0.5 p-2'>
        ...
        {file.status === WatermarkStatus.Failed && file.errorMessage && (
...
export default memo(WatermarkFileCard);
