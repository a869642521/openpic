...
import ConvertFileCard from './convert-file-card';
import useConvertStore from '@/store/convert';
import ConvertOptionsCard from './convert-options-card';
...
import { Plus, FileImage, LoaderPinwheel } from 'lucide-react';
...
function ConvertWorkspace() {
  ...
  useEffect(() => {
    if (!isValidArray(files)) navigate('/convert/guide');
  }, [files, navigate]);
  ...
  const handleAppendFiles = async (paths: string[] | null) => {
    ...
    const batchId = useConvertStore.getState().currentBatchTimestamp;
    const tagged = newFiles.map((f) => ({ ...f, batchId, status: ConvertStatus.Pending }));
    useConvertStore.getState().appendFiles(tagged);
  };
  ...
  const handleAddMoreClick = async () => {
    const selected = await open({
      multiple: true,
      directory: false,
      filters: [{ name: 'Image Files', extensions: GUIDE_IMAGE_AND_ARCHIVE_EXTS }],
    });
  ...
  const pendingFiles = files.filter(
    (f) => f.status === ConvertStatus.Pending || f.status === ConvertStatus.Failed,
  );
  const pendingCount = pendingFiles.length;
  const hasCompleted = files.some((f) => f.status === ConvertStatus.Completed);
  const disabledConvert = !files.length || inCompressing || pendingCount === 0;
  ...
  const handleConvert = async () => {
    ...
    const filesToProcess = allPending.filter((f) => f.ext?.toLowerCase() !== targetFormat);
    const skippedSameFormat = allPending.length - filesToProcess.length;

    if (filesToProcess.length === 0) {
      setInCompressing(false);
      message.info(
        skippedSameFormat > 0
          ? t('convert.same_format_skipped', { count: skippedSameFormat })
          : t('convert.no_files_to_process'),
      );
      return;
    }

    const alpha = /^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/.test(options.alpha?.trim() || '')
      ? options.alpha.trim()
      : '#FFFFFF';

    filesToProcess.forEach((f) => {
      updateFileItem(f.path, { status: ConvertStatus.Processing });
    });
    ...
    const msg =
      skippedSameFormat > 0
        ? t('convert.completed_with_skipped', {
            fulfilled,
            rejected,
            total: filesToProcess.length,
            skipped: skippedSameFormat,
          })
        : t('convert.completed', {
            fulfilled,
            rejected,
      ...
      >
        {isValidArray(dataList) ? (
          <div className='w-full px-3 pb-[65px]' style={{ paddingTop: '0.75rem' }}>
            ...
            >
              {dataList.map((file) => (
                <ConvertFileCard key={file.path} path={file.path} />
              ))}
            </div>
          </div>
        ) : (
      ...
      {isValidArray(files) && (
        <div className='flex min-w-0 flex-shrink-0 flex-col pb-0'>
          ...
          <div className='shrink-0 flex flex-col gap-2 pt-2'>
            ...
            >
              <p className='text-xs text-neutral-600'>{t('convert.file_count', { count: files.length })}</p>
            </div>
            <Button
              size='sm'
            ...
            >
              ...
              >
                ...
                <span>
                  {hasCompleted ? t('convert.convert_again', { count: pendingCount }) : t('convert.convert')}
                </span>
              </div>
            </Button>
            <Button
