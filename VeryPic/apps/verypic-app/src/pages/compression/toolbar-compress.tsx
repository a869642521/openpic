...
import { isValidArray, calProgress, formatCompressPercent } from '@/utils';
...
function ToolbarCompress() {
  ...
  const handleCompress = async () => {
    ...
    try {
      if (compressionMode === CompressionMode.Remote && !isValidArray(tinypngApiKeys)) {
      ...
      const effectiveCompressionType = CompressionType.Lossy;
      ...
      await new Compressor({
        ...
        (res) => {
          ...
          if (targetFile) {
            ...
            // 以体积差为准展示压缩率，避免 sidecar 把小收益四舍五入成 0 时 UI 误显示 0%
            const sizeRate =
              typeof res.input_size === 'number' &&
              res.input_size > 0 &&
              typeof res.output_size === 'number'
                ? (res.input_size - res.output_size) / res.input_size
                : 0;
            const displayRate = Math.max(res.compression_rate ?? 0, sizeRate);
            if (displayRate > 0) {
              ...
              targetFile.compressRate = `${formatCompressPercent(displayRate * 100)}%`;
