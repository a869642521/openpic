...
/** 工作区监听列表底部「添加文件夹」：全宽 + 灰描边（与首页监听引导页的固定宽按钮样式区分） */
const WATCH_LIST_ADD_BUTTON_CLASS =
  'h-12 w-full gap-2 rounded-full border border-neutral-300 bg-transparent text-base shadow-none transition-colors hover:bg-black hover:text-white hover:border-black dark:border-neutral-600 dark:bg-transparent dark:hover:bg-black dark:hover:text-white dark:hover:border-black';
...
function WatchFolderList() {
  const t = useI18n();
  const { watchFolders, setWorking, addWatchFolder, pendingWatchPath, setPendingWatchPath, lastAddedWatchFolderId, clearLastAddedWatchFolderId } = useCompressionStore(
    useSelector(['watchFolders', 'setWorking', 'addWatchFolder', 'pendingWatchPath', 'setPendingWatchPath', 'lastAddedWatchFolderId', 'clearLastAddedWatchFolderId']),
  );
  const { messageApi } = useContext(AppContext);
  ...
  // 渲染后清除 lastAddedWatchFolderId，避免后续重绘时误用
  useEffect(() => {
    if (lastAddedWatchFolderId && watchFolders.some((f) => f.id === lastAddedWatchFolderId)) {
      clearLastAddedWatchFolderId();
    }
  }, [lastAddedWatchFolderId, watchFolders, clearLastAddedWatchFolderId]);
    ...
    <div ref={dropzoneRef} className='flex h-full flex-col overflow-y-auto' style={{ backgroundColor: 'rgb(250, 250, 250)' }}>
      {watchFolders.length === 0 ? (
        ...
        <div className='flex w-full flex-1 flex-col items-center justify-center gap-4 p-8'>
          ...
          <Button
            variant='default'
            className='mt-2 h-12 w-[220px] max-w-[220px] shrink-0 flex-none cursor-pointer gap-2 rounded-full border-0 bg-black text-base text-white shadow-none hover:bg-neutral-800 dark:bg-black dark:text-white dark:hover:bg-neutral-800'
        ...
        <div className='flex flex-col gap-3 p-4'>
          {watchFolders.map((folder) => (
            <FolderCard
              key={folder.id}
              folder={folder}
              initialExpanded={folder.id === lastAddedWatchFolderId}
            />
          ))}

          {/* 添加更多文件夹按钮 */}
          <Button variant='secondary' onClick={() => handleAddFolder()} className={WATCH_LIST_ADD_BUTTON_CLASS}>
            <Plus size={18} />
