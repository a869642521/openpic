...
import { isValidArray, formatCompressPercent } from '@/utils';
...
function CompressionWatch() {
  ...
  const handleCompress = async (
    ...
    try {
      ...
      const compressionMode = folderSettings.compressionMode ?? CompressionMode.Auto;
      const compressionLevel = folderSettings.compressionLevel ?? 1;
        ...
        (res) => {
          ...
          if (targetFile) {
            ...
            if (res.compression_rate > 0) {
              ...
              targetFile.compressRate = `${formatCompressPercent(res.compression_rate * 100)}%`;
            } else {
  ...
  const startWatching = useCallback((folder: WatchFolder) => {
    ...
    fetchEventSource(`${sidecar.origin}/stream/watch/new-images`, {
      ...
      headers: {
        ...
        Authorization: `Bearer verypic_sidecar`,
      },
