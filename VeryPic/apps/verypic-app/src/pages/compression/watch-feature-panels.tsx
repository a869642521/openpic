...
export const CompressionPanel = memo(function CompressionPanel({
  ...
  const compressionMode = s.compressionMode ?? CompressionMode.Auto;
  ...
  return (
    <div className={cn(SECTION_CLASS, 'space-y-3')}>
      ...
      {modeTab === 'auto' && (
        <div className={ROW_CLASS}>
          <Label className={LABEL_CLASS}>{t('settings.compression.mode.title')}</Label>
          <Select
            value={compressionMode}
            onValueChange={(v) => onChange({ compressionMode: v as CompressionMode })}
          >
