...
function ClassicCompressionGuide() {
  ...
  const handleFiles = async (paths: string[] | null) => {
    ...
    const files = await parsePathsForGuideWithProgress(paths!, VALID_IMAGE_EXTS, {
      progressRef,
      t,
      emptyMessageKey: 'common.no_image_to_compress',
    });
    if (!files) return;
        ...
        <div className='flex flex-col gap-8 md:flex-row'>
          <div className='flex-1'>
            <div className='group relative flex flex-col items-center justify-center gap-4 p-5'>
              ...
              <div className='flex flex-col items-center gap-1 text-center sm:flex-row sm:flex-wrap sm:justify-center sm:gap-x-6 sm:gap-y-1'>
                <p className='text-xs text-neutral-400 dark:text-neutral-500'>
                  {t('page.compression.classic.paste_hint')}
                </p>
                <p className='text-xs text-neutral-400 dark:text-neutral-500'>
                  {t('page.compression.classic.archive_hint')}
                </p>
              </div>
            </div>
          </div>
