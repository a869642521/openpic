...
function WatermarkWorkspace() {
  ...
  const disabledAddWatermark =
    ...
    (isTile && !options.watermarkText?.trim() && !options.watermarkImagePath?.trim()) ||
  ...
  const handleAddWatermark = async () => {
    ...
    await addWatermark(
      filesToProcess,
      {
        ...
        tileGapX: options.tileGapX,
        tileGapY: options.tileGapY,
        tileRotation: options.tileRotation,
        positionNormX: options.positionNormX,
        positionNormY: options.positionNormY,
        concurrency: 6,
      },
      (res) => {
        fulfilled++;
