...
/** 拖入/选择压缩包时支持解压并按 VALID_IMAGE_EXTS 筛选内部图片
 *  ZIP：`zip` crate；RAR：内置 `unrar`（RARLAB 解压库静态链接），无需安装 WinRAR */
export const IMPORT_ARCHIVE_EXTS = ['zip', 'rar'] as const;

/** 引导页/追加文件对话框：图片 + 可导入压缩包 */
export const GUIDE_IMAGE_AND_ARCHIVE_EXTS = [...VALID_IMAGE_EXTS, ...IMPORT_ARCHIVE_EXTS];
