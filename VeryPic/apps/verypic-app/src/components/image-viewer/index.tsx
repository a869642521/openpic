...
const ImageViewer = forwardRef<ImageViewerRef, ImageViewerProps>(function ImageViewer(
  ...
  const sidecarOrigin = sidecar?.origin;
  ...
  // 当 sidecarOrigin 从无到有时，重置 hasRenderedRef 以触发缩略图生成
  useEffect(() => {
    if (useThumbnail && sidecarOrigin) {
  ...
  useEffect(() => {
    ...
    if (!useThumbnail || !sidecarOrigin || !imageTempDir || hasRenderedRef.current) {
      // sidecar 未就绪时先用原始 src 直接渲染，等 origin 到位后 effect 重新触发
      if (useThumbnail && !hasRenderedRef.current && !sidecarOrigin) {
        setDisplaySrc(src);
      }
      return;
    }
