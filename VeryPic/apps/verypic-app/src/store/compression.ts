...
interface CompressionState {
  ...
  /** 刚添加的监听文件夹 id，用于列表自动展开，渲染后清除 */
  lastAddedWatchFolderId: string | null;
}
...
interface CompressionAction {
  ...
  clearLastAddedWatchFolderId: () => void;
}
...
const useCompressionStore = create<CompressionState & CompressionAction>()(
  ...
  addWatchFolder: (folder: WatchFolder) => {
    ...
    set({ watchFolders: [...watchFolders, folder], lastAddedWatchFolderId: folder.id });
  },

  clearLastAddedWatchFolderId: () => {
    set({ lastAddedWatchFolderId: null });
  },
  ...
  resetWatchOnly: () => {
    ...
    {
      name: 'verypic-compression',
      storage: createJSONStorage(() => localStorage),
      partialize: (state) => ({
        watchFolders: state.watchFolders,
        classicSettings: state.classicSettings,
      }),
      /** 每次启动从存储恢复后：批量压缩「更多选项」一律恢复为未勾选（不随上次会话持久化） */
      merge: (persisted, current) => {
        const p = (persisted ?? {}) as Partial<CompressionState>;
        const next: CompressionState & CompressionAction = {
          ...(current as CompressionState & CompressionAction),
          ...p,
          classicSettings: {
            ...current.classicSettings,
            ...p.classicSettings,
            preserveMetadata: defaultClassicSettings.preserveMetadata,
            convertEnable: defaultClassicSettings.convertEnable,
            convertTypes: [...defaultClassicSettings.convertTypes],
            convertAlpha: defaultClassicSettings.convertAlpha,
            resizeEnable: defaultClassicSettings.resizeEnable,
            resizeMode: defaultClassicSettings.resizeMode,
            resizeScale: defaultClassicSettings.resizeScale,
            resizeDimensions: [...defaultClassicSettings.resizeDimensions],
            resizeFit: defaultClassicSettings.resizeFit,
          },
        };
        return next;
      },
