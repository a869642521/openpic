...
export async function addWatermark(
  ...
  const payload = {
    options: {
      ...
      tile_gap_x: options.tileGapX ?? 40,
      tile_gap_y: options.tileGapY ?? 40,
      tile_rotation: options.tileRotation ?? 0,
      ...(!isTile && {
        position_norm_x: options.positionNormX ?? 0.98,
        position_norm_y: options.positionNormY ?? 0.98,
      }),
    },
    save: {
