//! VeryPic Windows 11 IExplorerCommand COM 扩展
//!
//! 实现三个右键菜单动词：后台压缩、后台监听、打开设置。
//! 从 HKCU\Software\VeryPic\ContextMenu\ExePath 读取主程序路径。
...
impl VerbType {
    fn title(&self) -> &'static str {
        match self {
            VerbType::Compress => "VeryPic 后台压缩",
            VerbType::Watch => "VeryPic 后台监听",
            VerbType::Settings => "VeryPic 打开设置",
        }
    }
}
...
fn get_exe_path_from_registry() -> Option<Vec<u16>> {
    ...
    unsafe {
        ...
        let ret = RegOpenKeyExW(
            HKEY_CURRENT_USER,
            windows::core::w!("Software\\VeryPic\\ContextMenu"),
            None,
            KEY_READ,
            &mut hkey,
        );
...
struct VeryPicExplorerCommand {
    verb: VerbType,
}

impl VeryPicExplorerCommand {
    fn new(verb: VerbType) -> Self {
        Self { verb }
    }
}

impl windows::Win32::UI::Shell::IExplorerCommand_Impl for VeryPicExplorerCommand_Impl {
    fn GetTitle(
        &self,
...
struct VeryPicClassFactory {
    verb: VerbType,
}

impl VeryPicClassFactory {
    fn new(verb: VerbType) -> Self {
        Self { verb }
    }
}

impl windows::Win32::System::Com::IClassFactory_Impl for VeryPicClassFactory_Impl {
    fn CreateInstance(
        &self,
        ...
        let cmd = VeryPicExplorerCommand::new(self.verb);
...
pub unsafe extern "system" fn DllGetClassObject(
    ...
    *ppv = ptr::null_mut();
    ...
    let factory = VeryPicClassFactory::new(verb);
