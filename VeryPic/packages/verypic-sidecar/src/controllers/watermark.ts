...
import { z } from 'zod';
import sharp, { FormatEnum, Gravity } from 'sharp';
...
import { addTextWatermark, addImageWatermark, addTileWatermark } from '../services/watermark';
...
const PayloadSchema = z.object({
  ...
  options: z
    .object({
      ...
      /** 平铺水印：横向间距 (px) */
      tile_gap_x: z.number().int().min(0).optional().default(40),
      /** 平铺水印：纵向间距 (px) */
      tile_gap_y: z.number().int().min(0).optional().default(40),
      /** 平铺水印：旋转角度 (度) */
      tile_rotation: z.number().min(-360).max(360).optional().default(0),
      /** 锚点归一化坐标 0–1（相对原图），与拖拽预览一致 */
      position_norm_x: z.number().min(0).max(1).optional(),
      position_norm_y: z.number().min(0).max(1).optional(),
    })
    .optional()
    .default({} as any),
  save: z
...
app.post('/', zValidator('json', PayloadSchema, payloadValidator), async (context) => {
  ...
  if (
    (options.watermark_type === WatermarkType.Text || options.watermark_type === WatermarkType.Tile) &&
    options.watermark_text?.trim() === '' &&
    options.watermark_image_path?.trim() === ''
  ) {
  ...
  try {
    ...
    const gravityForSharp: Gravity =
      options.watermark_position === WatermarkPosition.Custom
        ? ('southeast' as Gravity)
        : (options.watermark_position as Gravity);

    const positionNorm =
      typeof options.position_norm_x === 'number' && typeof options.position_norm_y === 'number'
        ? { x: options.position_norm_x, y: options.position_norm_y }
        : undefined;

    if (options.watermark_type === WatermarkType.Text) {
      stream = await addTextWatermark({
        stream,
      ...
      stream = await addImageWatermark({
        stream,
    ...
    } else if (options.watermark_type === WatermarkType.Tile) {
      const contentType = options.watermark_image_path?.trim() ? 'image' : 'text';
      stream = await addTileWatermark({
        stream,
        contentType,
        text: options.watermark_text,
        color: options.watermark_text_color,
