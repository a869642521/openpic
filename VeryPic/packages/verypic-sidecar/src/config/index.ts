...
export async function loadConfig(): Promise<AppConfig> {
  ...
  const cluster = parseBoolean(process.env.VERYPIC_SIDECAR_CLUSTER, false);
  const port = await findAvailablePort(isDev ? 3000 : undefined);
  const mode =
    (String(process.env.VERYPIC_SIDECAR_MODE || 'server').toLowerCase() as AppConfig['mode']) ||
    'server';
  let store: Record<string, unknown> = {};
  if (process.env.VERYPIC_SIDECAR_STORE) {
    try {
      store = JSON.parse(String(process.env.VERYPIC_SIDECAR_STORE)) as Record<string, unknown>;
    } catch (e) {
